...
namespace Relativity.Server.Import.SDK.Samples.Tests
{
	...
	public abstract class DocImportTestsBase : ImportTestsBase
	{
		...
		protected DocImportTestsBase()
			: base(FunctionalTestHelper.Logger)
		{
		...
		protected static string GenerateFolderPath(int maxDepth)
		{
			...
			{
				string folderName = $"\\{Guid.NewGuid()}-{RandomHelper.NextString(20, FunctionalTestParameters.MaxFolderLength - 36)}";
		...
		protected void AssertImportSuccess()
		{
			...
			// Always check for fatal exceptions first; otherwise, assert failures might not make sense.
